...
|--------|-----------------------------------|------|---------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| GET    | `/transactions`                   | JWT  | `direction` (enum), `special_category` (enum), `account_id`, `from`, `to`, `limit`, `cursor` | Cursor-paginated list of transactions. `from` is inclusive, `to` is exclusive (half-open `[from, to)` — see Aggregation section). `direction` and `special_category` are typed as `StrEnum` (canonical case only — `?direction=expense` works, `?direction=Expense` returns 422). Each item includes `currency_code` (account base currency), `operation_currency_code` (merchant currency, nullable), `direction` (`income`/`expense`/`zero`), and `special_category` (NULL for ordinary, `'transfer'` for internal movements). |
...
4. Reprocess coordination (the pipeline consumer is decoupled from reprocess by design — see `adr-transaction-reprocessing.md` "Concurrency"). The pipeline consumer never blocks on the reprocess advisory lock. Coordination happens one stage upstream: the **normalization consumer** wraps its publish-to-`normalized_transactions` step in a check against `reprocessing_locks` for the message's `user_id`. If a lock row exists, the normalizer INSERTs the `NormalizedTransaction` into the `staging_normalized_transactions` buffer table instead of publishing to Kafka. Both the read (lock row check) and the write (publish or stage) commit in the same transaction so the routing decision is consistent. The pipeline consumer continues to process whatever lands on `normalized_transactions` without coordination overhead. Reprocess takes a **session-scoped** `pg_advisory_lock(hashtext('reprocess:' || user_id::text))` for the entire job — see §2.1's reprocessing-job paragraph.
...
1. **MCC + idempotency gate** — return immediately if `mcc != '4829'` (cheap in-memory check) or if `tx.id` already exists.
2. **Precompute row flags** — three values bound for the rest of the pass: `description_matched: bool`, `multi_hop_description: bool`, and `cp_iban_status ∈ {null, transitive, external, honest}`. The directional transitive rule (multi-hop expense IBANs are suppressed because they point at the chain end, not the immediate partner) is encoded here as a single flag transition — no scattered `if direction == 'expense'` branches downstream.
3. **External-IBAN short-circuit** (the only flag-driven branch) — `cp_iban_status == 'external'` rows skip the universal fetch entirely. They write `metadata.layer.transfer.row` for traceability and may emit `unpaired_*_description` if the prefix-but-unknown drift signal applies.
...
5. **Hard IBAN consistency filter** — drops candidates whose `honest`/`external` `cp_iban` contradicts the incoming row's account. `null`/`transitive` impose no constraint. `external` candidates are always dropped (their cp_iban definitionally resolves to no own account).
...
| `sources/monobank/transfer/flags.py`            | `RowFlags` dataclass + `compute_row_flags(tx, account_iban_lookup) -> RowFlags`. Pure function. Owns the directional transitive rule (`expense AND multi_hop_description → cp_iban_status='transitive'`) as a single explicit branch. | ~60        |
...
| `sources/monobank/transfer/decision.py`         | `decide(candidates, incoming_flags, account_props) -> Decision` — the count-and-decide branch (step 7). Returns one of `Claim(partner_id, iban_evidence, description_decisive)`, `Anomaly(code, candidate_ids, detail)`, or `Skip()` (a discriminated union). `iban_evidence` is the same enum as the one written into `metadata.layer.transfer.pair.iban_evidence` — kept as a single name across the strategy so a reader can grep one term to trace the field from decision to JSON. The bucket-locked principle lives here as a single `_select_bucket(candidates_by_evidence)` helper. | ~140       |
...
| `repositories/transfer_repo.py`                 | SQL repo. Methods: `exists(tx_id)`, `find_universal_candidates(...)`, `claim_pair(existing_id, new_id, pair_metadata_block)`. SQL is triple-quoted, one item per line per project SQL style. | ~100       |
| `repositories/account_property_repo.py`         | `get_account_with_properties`, `get_accounts_with_properties`, `get_user_account_by_iban`. The IBAN-pointing-at-own-account lookup is what backs the consistency filter and evidence classification. | ~70        |
...
**Orchestrator merge contract.** The pipeline orchestrator (`services/pipeline.py`) is the single place that writes `metadata.layer.<name>` sub-blocks onto a row. The contract:
...
**Ingestion service triggers all three K8s Jobs** (transactions backfill, rates backfill, reprocess) via the `kubernetes` Python client. `BackfillService` and `ReprocessDispatcher` both construct `V1Job` objects programmatically with unique timestamped names and pass all parameters as container env vars alongside `grosh-secrets` via `envFrom`. The `kubernetes` package is a dependency of the ingestion service.
...
| `services/consumer/src/grosh_consumer/services/transfer_detection.py`  | `TransferDetectionStrategy` Protocol + `TransferResult`/`AnomalyRecord` dataclasses (the strategy interface — implementations live under `sources/{bank}/transfer/`) |
...
| `services/consumer/src/grosh_consumer/sources/monobank/transfer/flags.py`           | `RowFlags` dataclass + `compute_row_flags()`. Owns the directional transitive rule for `cp_iban_status`. Pure function. |
| `services/consumer/src/grosh_consumer/sources/monobank/transfer/iban_classifier.py` | `is_consistent()` hard-filter check + `classify_pair_evidence()` returning `bilateral`/`unilateral`/`none`. Pure functions. |
| `services/consumer/src/grosh_consumer/sources/monobank/transfer/decision.py`        | `decide()` count-and-decide branch. Owns the bucket-locked principle (`_select_bucket()`). Returns typed `Claim`/`Anomaly`/`Skip` discriminated union. |
| `services/consumer/src/grosh_consumer/sources/monobank/transfer/metadata.py`        | `build_row_block()`, `build_pair_block()` — pure JSON-shape builders for `metadata.layer.transfer.{row,pair}`. Centralized so JSON shape changes touch one file. |
| `services/consumer/src/grosh_consumer/sources/monobank/transfer/anomalies.py`       | One builder per `AnomalyRecord` variant. Centralizes `reason_detail` formatting; the detector never builds anomalies inline. |
| `services/consumer/src/grosh_consumer/sources/monobank/descriptions.py`             | Description guard maps and validators: `_INCOME_MAP`, `_EXPENSE_MAP`, `parse_description`, `is_transfer_description`, `is_multi_hop_description` (the `для переказу на` family flag), `validate_pair_descriptions`. |
...
| `services/consumer/src/grosh_consumer/services/currency_conversion_service.py` | `ConversionResult` dataclass + source-agnostic rate resolution service |
| `services/consumer/src/grosh_consumer/repositories/account_repo.py`    | Account lookups (reads only — `get_currency_code` and other thin queries used by the orchestrator) |
| `services/consumer/src/grosh_consumer/repositories/account_property_repo.py` | Account property queries used by transfer detection (`get_account_with_properties`, `get_user_account_by_iban`, batch variant). Separate from `account_repo.py` to keep the property-rich queries scoped to the strategy module. |
| `services/consumer/src/grosh_consumer/repositories/transfer_repo.py`   | Transfer detection SQL repo. Methods: `exists(tx_id)`, `find_universal_candidates(...)`, `claim_pair(existing_id, new_id, pair_metadata_block)`. |
...
| `services/consumer/src/grosh_consumer/services/staging_drain_service.py` | `StagingDrainService` — long-lived asyncpg `LISTEN reprocess_complete` connection + 60s periodic sweep fallback. On notification, calls `drain_for_user(user_id)`: select staged rows in `created_at` order, publish each to `normalized_transactions`, DELETE after each successful publish (publish-then-delete for at-least-once). Lives under `services/` (not `consumers/`) because it is NOT a Kafka consumer — it never calls `consumer.poll`; it's a stateful background coordinator (NOTIFY listener + sweep timer + producer + dedicated DB connection) that happens to share the consumer process for cost reasons. All SQL goes through `StagingRepo`. Process placement: shares the consumer process; the normalization consumer does NOT import or own it. **Lifecycle is async context manager** (`async with StagingDrainService(pool, staging_repo): ...`): `__aenter__` constructs the Kafka producer (NOT in `__init__` — that would leak the producer if the instance were constructed but never entered), spawns the listener + sweep background tasks, and returns `self`. `__aexit__` runs the load-bearing 5-step cleanup in this exact order (each step in its own `try/except` so a failure in step N does not skip steps N+1..N+4): (1) `listener_task.cancel()` and `sweep_task.cancel()`; (2) `await asyncio.gather(listener_task, sweep_task, return_exceptions=True)` — lets each task's `finally` block run, which is what closes the listener connection cleanly via the `finally` in `_connect_and_listen`; (3) defensive idempotent close of `self._listener_conn` in case step 2's `finally` didn't run; (4) `await asyncio.wait_for(asyncio.gather(*self._notify_drain_tasks, ...), timeout=2.0)` — waits briefly for in-flight NOTIFY-driven drains to publish their pending rows, then cancels any that haven't completed; (5) `self._producer.flush(timeout=10)` — flush LAST so in-flight publishes from steps 2 and 4 land before the producer goes away. Reversing this order would risk asyncpg double-close errors and silent message loss. **Shutdown is cancellation-driven, not signal-driven**: the listener loops on `while True` with `await asyncio.Future()` as the park point inside `_connect_and_listen`, and the sweep loops on `while True` with `await asyncio.sleep(_SWEEP_INTERVAL_SECONDS)`. `task.cancel()` raises `CancelledError` through whichever await is currently parked, which propagates through each task's `finally` block (no soft-stop `Event` indirection — the previous design held a `_stop = asyncio.Event()` field that was awaited but never `.set()`-ed, dead state confusing readers; removed in favour of cancellation as the single shutdown mechanism). **NOTIFY-driven drain tasks are tracked** in `self._notify_drain_tasks: set[asyncio.Task]` (initialized in `__init__` because `_on_notify` could fire before `__aenter__` completes if the listener task starts up first); `_on_notify` schedules each drain via `task = asyncio.create_task(...); self._notify_drain_tasks.add(task); task.add_done_callback(self._notify_drain_tasks.discard)` so step 4 of `__aexit__` can wait for them. This closes the previous fire-and-forget gap (`asyncio.ensure_future(...)`) where in-flight drains leaked past shutdown and silently swallowed exceptions. **No test-injection in production signatures**: `_sweep_once` and `drain_for_user` take only the implicit pool — no `_conn: asyncpg.Connection | None = None` kwarg branching on test-only state. Tests inject a `SingleConnectionPool` wrapper (test helper at `services/consumer/tests/integration/conftest.py`) whose `acquire()` always yields the test's rolled-back-transaction conn. NOT an `asyncpg.Pool(min_size=1, max_size=1)` — that would create a separate connection invisible to the test's transaction. |
| `services/consumer/src/grosh_consumer/jobs/run_reprocess.py`           | Thin K8s Job entrypoint (~50 lines). Parses `USER_IDS_JSON` env var, opens the dedicated `asyncpg.connect()` (NOT pool — session-scoped advisory lock must survive across calls), constructs `ReprocessRepo` + Kafka `Producer` + `ReprocessService`, iterates user IDs sequentially calling `service.reprocess_user(uid)`, closes the connection in `finally`. No SQL, no orchestration logic. |
