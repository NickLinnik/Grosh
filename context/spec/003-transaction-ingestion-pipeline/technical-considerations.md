...
| `services/consumer/src/grosh_consumer/repositories/account_repo.py`    | Single account repository. Methods return a frozen `AccountProps` dataclass (`id`, `type`, `currency_code`, `iban`) — no raw tuples. Surface: `get_by_id(conn, account_id) -> AccountProps` (raises `AccountNotFoundError`), `get_many_by_ids(conn, account_ids) -> dict[UUID, AccountProps]` (returns only accounts that exist — callers handle missing IDs via `.get(id)`), `find_by_iban(conn, iban, user_id) -> AccountProps \| None`. Covers all consumer-side account reads: orchestrator's account-currency lookup, transfer detection's IBAN-to-account resolution, batch lookup for the bucket-locked decision. |
