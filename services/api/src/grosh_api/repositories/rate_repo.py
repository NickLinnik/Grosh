...
class RateRepo:
    ...
    async def count_rates(
        ...
        *,
        ...
        count: int = await conn.fetchval(
            f"SELECT COUNT(*) FROM currency_rates {where_clause}", *params
        )
        return count
    ...
    async def list_rates(
        ...
        *,
        ...
        if cursor_valid_from is not None and cursor_id is not None:
            ...
            where = f"{where} AND {cursor_cond}" if where else cursor_cond
    ...
    async def get_rates_at(
        ...
        *,
        ...
        # `at` appears twice — once for the valid_from upper bound, once for the SCD2
        # valid_to lower bound. Postgres binds each $N independently.
        where, params = build_where(
            [
                ("valid_from <=", at),
