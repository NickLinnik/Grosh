...
strings live here. Publishing is delegated to ReplayService. The same
instance is reused across all users in one job run.
"""
...
class ReprocessOrchestrator:
    ...
    async def reprocess_user(self, conn: asyncpg.Connection, user_id: UUID) -> bool:
        ...
        try:
            ...
            published = self._replay_service.publish_normalized_events(user_id, events)
