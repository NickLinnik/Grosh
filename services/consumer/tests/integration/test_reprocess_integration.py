...
from grosh_consumer.services.reprocess_service import (
    ReprocessService,
)
...
async def test_catchup_zero_expected_returns_immediately(
    ...
    service = ReprocessService(_repo, MagicMock(), mock_producer)
    await service._wait_for_pipeline_catchup(conn, user_id, expected_count=0)
...
async def test_catchup_returns_when_count_matches(conn: asyncpg.Connection) -> None:
    ...
    mock_producer = MagicMock()
    service = ReprocessService(_repo, MagicMock(), mock_producer)
    await service._wait_for_pipeline_catchup(conn, user_id, expected_count=1)
...
async def test_catchup_times_out_when_count_never_reached(
    ...
    import grosh_consumer.services.reprocess_service as mod
    ...
    with pytest.raises(ReprocessError, match="catchup timeout"):
        await service._wait_for_pipeline_catchup(conn, user_id, expected_count=999)
...
async def test_publish_replay_events_calls_produce_for_each_event() -> None:
    """_publish_replay_events calls producer.produce once per event."""
    events = [make_event() for _ in range(3)]
    mock_producer = MagicMock()
    service = ReprocessService(_repo, MagicMock(), mock_producer)

    count = service._publish_replay_events(events)
...
async def test_publish_replay_events_empty_list() -> None:
    """_publish_replay_events with no events produces nothing and returns 0."""
    mock_producer = MagicMock()
    service = ReprocessService(_repo, MagicMock(), mock_producer)

    count = service._publish_replay_events([])
...
async def test_reprocess_recovers_on_publish_failure(
    ...
    try:
        ...
        service = ReprocessService(_repo, TransactionRepo(), mock_producer)
...
async def test_run_reprocess_continues_after_one_user_fails() -> None:
    ...
    mock_service.reprocess_user = AsyncMock(side_effect=[boom, None])
    ...
    with (
        patch(
            "grosh_consumer.jobs.run_reprocess.asyncpg.connect",
            new=AsyncMock(return_value=mock_session_conn),
        ),
        patch(
            "grosh_consumer.jobs.run_reprocess.Producer",
            return_value=MagicMock(),
        ),
        patch(
            "grosh_consumer.jobs.run_reprocess.ReprocessRepo",
            return_value=mock_repo,
        ),
        patch(
            "grosh_consumer.jobs.run_reprocess.TransactionRepo",
            return_value=MagicMock(),
        ),
        patch(
            "grosh_consumer.jobs.run_reprocess.ReprocessService",
            return_value=mock_service,
        ),
    ):
        from grosh_consumer.jobs.run_reprocess import run_reprocess

        await run_reprocess(user_ids=[user_a, user_b])

    # Both users must have been attempted even though the first raised.
    assert mock_service.reprocess_user.call_count == 2
    calls = [c.args[1] for c in mock_service.reprocess_user.call_args_list]
