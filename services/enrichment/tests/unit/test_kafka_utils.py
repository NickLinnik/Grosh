"""Unit tests for grosh_enrichment.kafka utility functions.

Mirrors services/normalization/tests/unit/test_kafka_utils.py — both services
share the same poll-loop resilience model via grosh_shared.messaging.kafka_resilience
and need parallel coverage so a future refactor of either side surfaces in CI.
...
from grosh_enrichment.kafka import on_delivery, poll_message
...
async def test_poll_message_returns_none_on_partition_eof() -> None:
    """EOF is expected at partition boundaries — must return None, not raise."""
...
async def test_poll_message_raises_on_non_eof_error() -> None:
    """Non-EOF, non-tolerated errors must escalate as KafkaException."""
...
async def test_poll_message_returns_none_when_consumer_poll_returns_none() -> None:
    """Consumer timeout (no message available) must return None, not raise."""
...
async def test_poll_message_returns_value_and_message_on_success() -> None:
    """Happy path: a valid message returns the (raw_value, msg) tuple."""
...
async def test_poll_message_tolerates_unknown_topic_within_budget(
    ...
    """UNKNOWN_TOPIC_OR_PART within tolerance logs+returns None, not raise."""
    from grosh_enrichment import kafka as kafka_module
...
async def test_on_delivery_logs_error_and_does_not_raise(
    ...
    """Delivery callback runs in the producer thread — must log, not raise."""
    caplog.set_level(logging.ERROR, logger="grosh_enrichment.kafka")
